...
class PortfolioEnv(gym.Env):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.static_features = static_features.values.flatten() if static_features is not None else np.zeros(self.n_assets * 10)
        ...
        try:
            ...
            if model is None:
                model = PPO(
                    ...
                    learning_rate=5e-5,            # Más pequeño para menos oscilación
                    n_steps=256,                   # Más pasos por rollout = más contexto
                    batch_size=64,
                    gamma=0.99,
                    gae_lambda=0.92,
                    clip_range=0.2,                # Menos agresivo que 0.1
                    ent_coef=0.005,                # Menor entropía para menos exploración aleatoria
